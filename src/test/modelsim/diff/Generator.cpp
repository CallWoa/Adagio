...
*------------------------------------------------------------------------*/
    ...
    for(int tg = 0; tg < 8; ++tg){
        ...
        for(int step = 0; step < MAX_STEP; ++step){
            fprintf(fid,"step: %d\n", step);
            for(int i = 0; i < 8; ++i){
                fprintf(fid,"%lx ", ref_matrix_d_ulong[tg][step][i]);
                if(i % 4 == 3){
...
void tensor :: genfile(){
    FILE * fid = fopen("./src/matrix_a.txt","w");
    if(fid == NULL){
        printf("OPEN FILE FAILED!\n");
        return;
    }
    for(int i = 0; i < 16; ++i){
        for(int j = 0; j < 16; ++j){
            if(j == 15){
                fprintf(fid,"%lx\n", __matrix_a[i][j].to_ulong());
            }else{
                fprintf(fid,"%lx ", __matrix_a[i][j].to_ulong());
            }
        }
    }
    fclose(fid);

    fid = fopen("./src/matrix_b.txt","w");
    if(fid == NULL){
        printf("OPEN FILE FAILED!\n");
        return;
    }
    for(int i = 0; i < 16; ++i){
       for(int j = 0; j < 16; ++j){
            if(j == 15){
                fprintf(fid,"%lx\n", __matrix_b[i][j].to_ulong());
            }else{
                fprintf(fid,"%lx ", __matrix_b[i][j].to_ulong());
            }
       }
    }
    fclose(fid);

    fid = fopen("./src/matrix_c.txt","w");
    if(fid == NULL){
        printf("OPEN FILE FAILED!\n");
        return;
    }
    for(int i = 0; i < 16; ++i){
        for(int j = 0; j < 16; ++j){
            if(j == 15){
                fprintf(fid,"%lx\n", __matrix_c[i][j].to_ulong());
            }else{
                fprintf(fid,"%lx ", __matrix_c[i][j].to_ulong());
            }
        }
    }
    fclose(fid);

    fid = fopen("./src/matrix_src.txt","w");
    if(fid == NULL){
        printf("OPEN FILE FAILED!\n");
        return;
    }
